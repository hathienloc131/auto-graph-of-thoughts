...
class LanguageModelPrompter(Prompter):
    ...
    def __init__(self, lm: AbstractLanguageModel, problem_description: str, system_prompt="""You are a helpful guide that guides the user by instructing them on the NEXT TASK in details as possible based on the PREVIOUS TASK and RESULT OF PREVIOUS TASK. \n NEXT TASK must meet the given DESCRIPTION. \n Do not mention RESULT OF PREVIOUS TASK in your NEXT TASK. \n  If the PREVIOUS TASK is START, it mean that you have just start to instruct user. \n Only answering NEXT TASK without any additional text.""", json_format=True) -> None:
        super().__init__()
        self.lm = lm
    ...
    def split_prompt(self, state_dicts: Dict, **kwargs) -> tuple[str, str]:
        query = f"""<Description>Describe the NEXT TASK by dividing or spliting the PREVIOUS TASK and RESULT OF THE PREVIOUS TASK into exactly {state_dicts["num_split"]} smaller things that are the same as RESULT OF THE PREVIOUS TASK. </Description> \n PROBLEM: {self.problem_description} \n PREVIOUS TASK: {state_dicts["state"]} \n RESULT OF PREVIOUS TASK: {state_dicts["current"]} \n NEXT TASK:"""
