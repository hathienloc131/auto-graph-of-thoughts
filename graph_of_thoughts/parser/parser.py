...
class Parser(ABC):
    ...
    def parse_aggregation_answer(self, agg_config: Dict, states: List[Dict], texts: List[str]) -> Union[Dict, List[Dict]]:
        """
        Parse the response from the language model for a aggregation prompt.
        
        `Input`:

        - agg_config: `<Dict>` The Aggregation Operation's Config used to check number of response
